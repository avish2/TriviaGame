// $.fn.game = function () {
	...
	var quizOver = false;
	var answers = [];  // array of users answers
	...
	function displayQuestion (questions) {
		if ((userAnswers.correct + userAnswers.incorrect) <= 10) {
			...
			var choicesArr = t.questions[questionNum].choices;
			var buttonsArr = [];

			for (var i = 0; i < choicesArr.length; i++) {
				$("#answer").append("<div class='button'>" + choicesArr[i] + "</div><br>");
				// console.log(questions[questionNum].correct);
			...
			quizOver = true;
			$("#textbox").append($("<div>", {
				text: "Unanswered: " + (10 - (t.userAnswers.correct + t.userAnswers.incorrect)),
				class: "result"
			}, "</div>"));
	...
	// wrong.play();	//plays audio when wrong
	// right.play();	//plays audio when right
